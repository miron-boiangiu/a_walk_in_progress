...
def check_user_info(username, email, password, password2):
	...
	if not any(char.isdigit() or char.islower() or char.isupper() or\
		char in SpecialSym for char in password):
		return 0
	return 1
...
def profile():
	...
	conn = get_db_connection()
	command = 'SELECT * FROM trips WHERE user="{}"'.format(session.get('username'))
...
def endtrip():
	...
	conn = get_db_connection()
	command = 'SELECT * FROM trips WHERE user="{}" AND trip_id={}'.format(session.get('username'), current_game_id)
	existing_users = conn.execute(command).fetchall()
	...
	print("Adding game")
	conn.execute('INSERT INTO trips (user, trip_id, name) VALUES (?, ?, ?)',\
...
def signup():
	if request.method == "POST":
		...
		if(valid_data):
			conn = get_db_connection()
			command = 'SELECT * FROM users WHERE username="{}" OR mail="{}"'.format(username, email)
			print(command)
			existing_users = conn.execute(command).fetchall()
			if(len(existing_users) != 0):
				conn.close()
			...
			conn.execute('INSERT INTO users (username, password, mail) VALUES (?, ?, ?)',\
...
def login():
	if request.method == "POST":
		...
		password = request.form.get("psw", "")
		valid_data = check_user_info(username, "test_email@test.com", password, password)

		if(valid_data):

			conn = get_db_connection()
			command = 'SELECT * FROM users WHERE username="{}" OR mail="{}"'.format(username, username)
			print(command)
			existing_users = conn.execute(command).fetchall()
			if(len(existing_users) == 0):
				conn.close()
...
def game():
	...
	if(current_quest):
		...
		return render_template("Game.html", quest=quest[0], current_quest=current_quest, got_request=got_request, all_quests=all_quests)
